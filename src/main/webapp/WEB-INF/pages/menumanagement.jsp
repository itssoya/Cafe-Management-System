		...
		<aside class="sidebar">
			<div class="sidebar-content">
				...
				<ul class="category">
					...
					<li><a
						href="${pageContext.request.contextPath}/usermanagement"> <img
		...
		<div class="main-content">
					...
					<div class="search-bar">
						<form action="${pageContext.request.contextPath}/menumanagement"
							...
							<i class="fa-solid fa-magnifying-glass"></i> <input type="text"
								name="search" value="${search}" placeholder="Find an item...">
			...
			<div class="section-card">
				<div class="section-header">
					<div>
						...
						<p>Organise your menu items into distinct groups for easier
							navigation.</p>
					</div>
					<a href="${pageContext.request.contextPath}/addcategory"
						class="btn-add-category"> <i class="fa-solid fa-plus"></i> Add
						Category
					</a>
				</div>
					...
					<c:forEach var="cat" items="${categories}">
						<div class="category-chip">
							...
							<div class="chip-actions">
								<a
									href="${pageContext.request.contextPath}/editcategory?id=${cat.categoryId}"
									class="chip-btn"> <i class="fa-solid fa-pen"></i>
								</a>

								<form action="${pageContext.request.contextPath}/menumanagement"
									method="post" style="display: inline"
									onsubmit="return confirm('Delete ${cat.categoryName}?')">
									<input type="hidden" name="action" value="removeCategory">
									<input type="hidden" name="categoryId"
										value="${cat.categoryId}">
									<button type="submit" class="chip-btn chip-btn-delete">
			...
			<div class="menu-controls">
				<div class="filter-tabs">
					<button class="tab active" onclick="filterTab(this, 'all')">All
						Items</button>
					<c:forEach var="cat" items="${categories}">
						<button class="tab"
							onclick="filterTab(this, '${cat.categoryName}')">
							${cat.categoryName}</button>
					</c:forEach>
				</div>

				<a href="${pageContext.request.contextPath}/addmenu"
					class="btn-new-entry"> <i class="fa-solid fa-plus"></i> Add
					Menu
				</a>
			</div>
			...
			<div class="table-card">
				<table>
					...
					<tbody>
						<c:choose>
									...
									<tr class="menu-row" data-category="${item.categoryName}">
										...
										<td>
											<div class="item-cell">
												...
												<div class="item-info">
													<span class="item-name">${item.itemName}</span> <span
										...
										<td>
											<div class="action-btns">
												...
												<div class="addRecipe">
													<a
														href="${pageContext.request.contextPath}/addrecipe?menuItemId=${item.itemId}&clear=true"
														class="btn-recipe" title="Edit Recipe"> <i
														class="fa-solid fa-book-open"></i> Recipe
													</a>
												</div>
												<form
													action="${pageContext.request.contextPath}/menumanagement"
													method="post" style="display: inline"
							...
							<c:otherwise>
								<tr>
									<td colspan="4" class="empty-state"><i
										class="fa-solid fa-mug-hot"></i>
										<p>No menu items found.</p></td>
								</tr>
							</c:otherwise>
						</c:choose>
					</tbody>
				...
				<c:if test="${totalPage > 0}">
					<div class="pagination">
						<c:choose>
							<c:when test="${currentPage > 1}">
								<a
									href="${pageContext.request.contextPath}/menumanagement?page=${currentPage - 1}&search=${search}"
									class="page-btn"> <i class="fa-solid fa-chevron-left"></i>
								</a>
							</c:when>
							<c:otherwise>
						...
						<c:forEach begin="1" end="${totalPage}" var="i">
							<c:choose>
								...
								<c:when
									test="${i == 1 || i == totalPage || (i >= currentPage - 2 && i <= currentPage + 2)}">
									<a
										href="${pageContext.request.contextPath}/menumanagement?page=${i}&search=${search}"
										class="page-btn">${i}</a>
								</c:when>
						...
						<c:choose>
							<c:when test="${currentPage < totalPage}">
								<a
									href="${pageContext.request.contextPath}/menumanagement?page=${currentPage + 1}&search=${search}"
									class="page-btn"> <i class="fa-solid fa-chevron-right"></i>
								</a>
							</c:when>
							<c:otherwise>
								<span class="page-btn disabled"><i
					...
					<div class="pagination-info">Showing page ${currentPage} of
						${totalPage}</div>
				</c:if>
			</div>

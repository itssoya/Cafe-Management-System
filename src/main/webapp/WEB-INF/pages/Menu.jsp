...
.modal-qty-input::-webkit-outer-spin-button, .modal-qty-input::-webkit-inner-spin-button
	{
...
<body>
	...
	<nav>
		...
		<div style="display: flex; align-items: center; gap: 12px;">
			<a href="${pageContext.request.contextPath}/cart"
				class="cart-nav-btn"> <i class="fa-solid fa-bag-shopping"></i>
	...
	<section class="hero-section">
		<div class="hero-left">
			...
			<p>Check our variety of freshly brewed Coffee and a wide range of
				Bakery items from BrewBakes</p>
	...
	<section class="menu-section" id="menu">
		...
		<div class="category-tabs">
			<button class="cat-tab active" onclick="filterCategory(this, 'all')">All
				Items</button>
			<c:forEach var="cat" items="${categories}">
				<button class="cat-tab"
					onclick="filterCategory(this, '${cat.categoryName}')">
			...
			<c:choose>
				<c:when test="${not empty menuItems}">
					<c:forEach var="item" items="${menuItems}">
						...
						<!-- CARD — links to modal -->
						<a href="#modal-${item.itemId}" class="menu-card"
							data-category="${item.categoryName}"> <c:choose>
							...
							<div class="card-body">
								<span class="card-category">${item.categoryName}</span> <span
									class="card-name">${item.itemName}</span>
							</div>

							<div class="card-footer">
								<span class="card-price"> Rs.<fmt:formatNumber
										value="${item.price}" minFractionDigits="2"
										maxFractionDigits="2" />
								</span> <span class="card-view"> View <i
									class="fa-solid fa-arrow-right" style="font-size: 0.7rem;"></i>
								</span>
							</div>
						...
						<div class="modal-overlay" id="modal-${item.itemId}">
							<div class="modal-box">
								...
								<a href="#" class="modal-close" title="Close"> <i
								...
								<div class="modal-details">
									...
									<div class="modal-price">
										Rs.
										<fmt:formatNumber value="${item.price}" minFractionDigits="2"
											maxFractionDigits="2" />
									</div>
										...
										<c:when test="${not empty currentUser}">
											<form action="${pageContext.request.contextPath}/cart"
												...
												<input type="hidden" name="menuItemId"
													...
													name="itemName" value="${item.itemName}"> <input
													type="hidden" name="price" value="${item.price}"> <input
													type="hidden" name="imageUrl" value="${item.imageUrl}">
												...
												<div class="modal-qty-row">
													<div class="modal-qty-controls">
														...
														<input type="number" id="qty-${item.itemId}"
															name="quantity" value="1" min="1" class="modal-qty-input">
														<button type="button" class="modal-qty-btn"
															onclick="changeQty('qty-${item.itemId}',1)">+</button>
													</div>
												</div>
										...
										<c:otherwise>
											<div class="modal-login-prompt">
												...
												<a href="${pageContext.request.contextPath}/login"
													class="btn-modal-login"> <i class="fa-solid fa-lock"></i>
													Login to Order
												</a>
											</div>
										</c:otherwise>
			...
			<div class="menu-pagination">
				<c:choose>
					<c:when test="${currentPage > 1}">
						<a
							href="${pageContext.request.contextPath}/menu?page=${currentPage - 1}&search=${search}"
							class="pg-btn"><i class="fa-solid fa-chevron-left"></i></a>
					</c:when>
					<c:otherwise>
						<span class="pg-btn disabled"><i
				...
				<c:forEach begin="1" end="${totalPage}" var="i">
					<c:choose>
						...
						<c:otherwise>
							<a
								href="${pageContext.request.contextPath}/menu?page=${i}&search=${search}"
								class="pg-btn">${i}</a>
						</c:otherwise>
					</c:choose>
				</c:forEach>
